	...
	({
		...
		create : function ( ) {
            ...
            // "40 is the size of the font, 50 is the offset"
            ...
            let half = this.__CONFIG_WIDTH/2;
            let quarter = half / 2;
            let halfLeft = half - quarter;
            let halfRight = half + quarter;
            this.add.rectangle(half,0, 1, 2000, 0xffffff).setOrigin(0,0); // half the screen
            this.add.rectangle(halfLeft, 0, 1, 2000, 0xffffff).setOrigin(0,0);
            this.add.rectangle(halfRight, 0, 1, 2000, 0xffffff).setOrigin(0,0);
            ...
            this.add.rectangle(halfLeft - 150/2, this.__arrowLeftY[0]-250, 150, 150, 0x00ff00).setOrigin(0,0); // where I think it should go
            this.add.rectangle(halfRight - 150/2, this.__arrowRightY[0]-250, 150, 150, 0x00ff00).setOrigin(0,0); // where I think it should go
			let myArrowLeft = this.add.text ( halfLeft - 150/2, this.__arrowLeftY [ 0 ] - 250, this.__textArray [ 0 ], {
				fontSize : 150,
			});
			let myArrowRight = this.add.text ( halfRight-(150/2), this.__arrowLeftY [ 0 ] - 250, this.__textArray [ 0 ], {
				fontSize : 150,
			});

            //var graphics = this.add.graphics();
            //graphics.lineStyle(2, 0xffff00, 1);
            //graphics.strokeRect(50, 50, 300, 200);
